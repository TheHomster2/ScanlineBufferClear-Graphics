...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  ...
  double y0 = points -> m[1][top]; 
  double z0 = points -> m[2][top];
  double x1 = points -> m[0][middle]; 
  double y1 = points -> m[1][middle]; 
  double z1 = points -> m[2][middle];
  double x2 = points -> m[0][bottom]; 
  double y2 = points -> m[1][bottom]; 
  double z2 = points -> m[2][bottom];
  ...
  d0 = (xtop - xbottom) / (ytop - ybottom);
  if(ymiddle - ybottom != 0){ 
    d1 = (xmiddle - xbottom)/(ymiddle - ybottom);
  }
  d2 = (xtop - xmiddle) / (ytop - ymiddle);


  dZ = (ztop - zbottom) / (ytop - ybottom);
  if(ymiddle - ybottom != 0){
    dZ0 = (zmiddle - zbottom) / (ymiddle - ybottom);
  }
  dZ1 = (ztop - zmiddle) / (ytop - ymiddle);

  for(y = ybottom; y < ymiddle; y++){
